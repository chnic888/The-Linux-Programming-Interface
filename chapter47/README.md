...
- 使用System V的信号量的通常步骤如下
	- 使用`semget()`创建一个新的或者打开一个已存在的信号量集
...
- `nsems`参数在`semget()`创建和获取场景下有不同的含义
	- 在创建场景下，会指定集合中信号量的个数，其值必须大于0
	- 在获取场景下，其值必须小于等于集合数量的大小，且无法修改一个已存在信号量的数量
- `semflg` 是一个bit mask，用来指定新创建的信号量或者检查既有信号量的权限，还可以通过下述一个或多个标记取OR来控制操作
